...
async function readVersionNumberOnce() {
  try {
    const r = await fetch(`${SCOPE_PREFIX}js/app/version.js`, { cache: "no-store" });
  ...
  if (!NAMES_PROMISE) {
    NAMES_PROMISE = (async () => {
      ...
      const PRECACHE_URLS = [
        ...
        `${SCOPE_PREFIX}js/app/core.js?rev=${REV}`,
...
function isBypassPath(pathname){
  const p = pathname;
  return (
    p === `${SCOPE_PREFIX}js/app/version.js` ||
    p === `${SCOPE_PREFIX}js/firebase/firebase-init.js` ||
    p === `${SCOPE_PREFIX}js/firebase/firebase-config.js` ||
    p === `${SCOPE_PREFIX}js/app/theme-boot.js` ||

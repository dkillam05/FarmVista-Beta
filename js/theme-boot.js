// /FarmVista-Beta/js/theme-boot.js  — viewport + theme + firebase boot + AUTH GUARD + USER CONTEXT WARM
  ...
  __fvBoot.once('__FV_FIREBASE_CHAIN__', async () => {
    try{
      ...
      if (!window.FV_FIREBASE_CONFIG) {
        try {
          await __fvBoot.loadScript('/FarmVista-Beta/js/firebase-config.js', { defer:false, async:false });
        } catch(e) {
      ...
      if (!window.__FV_FIREBASE_INIT_LOADED__) {
        ...
        try {
          await __fvBoot.loadScript('/FarmVista-Beta/js/firebase-init.js', { type:'module', defer:true });
          console.log('[FV] firebase-init loaded');
        } catch (e) {
          console.warn('[FV] firebase-init failed to load — check path /FarmVista-Beta/js/firebase-init.js', e);
        }
      }
      ...
      try{
        ...
        if (!window.__FV_PERM_UI_SCRIPT_LOADED__) {
          ...
          try { await __fvBoot.loadScript('/FarmVista-Beta/js/perm-ui.js', { defer:true }); }
...
/* Provides FV.can("key") and FV.can("key:action") using FVUserContext perms.
   UI behavior (hide tiles / disable forms) is handled by /FarmVista-Beta/js/perm-ui.js.
*/
(function(){
  try{
    window.FV = window.FV || {};

/* /FarmVista-Beta/js/core/shell/fv-shell.js */
...
_hideBootOverlayWithOptionalHold(){
  try{
    ...
    async _bootSequence(){
      await this._loadScriptOnce('/FarmVista-Beta/js/core/version/version.js').catch(()=>{});
...
await this
  ._loadScriptOnce(
    '/FarmVista-Beta/js/shared/permissions/menu-acl.js'
  )
  .catch(() => {});
    ...
    async _ensureFirebaseInit(){
      try {
        if (!window.__FV_FIREBASE_INIT_LOADED__) {
          ...
          await this._loadScriptOnce('/FarmVista-Beta/js/core/firebase/firebase-init.js', { type:'module' });
        }
      } catch {}
    }
...
_applyCompanyBrand(
  ...
  if (
    ...
    async _loadMenu(){
      ...
      const primary = `${FV_ROOT}/FarmVista-Beta/js/shared/navigation/menu.js`;
      const fallback = `/FarmVista-Beta/js/shared/navigation/menu.js`;
  ...
  && !(document.body && document.body.classList.contains('fv-modal-open'));
    ...
    async checkForUpdates(){
      ...
      const readTargetVersion = async ()=>{
        try{
          const resp = await fetch('/FarmVista-Beta/js/core/version/version.js?ts=' + Date.now(), { cache:'reload' });

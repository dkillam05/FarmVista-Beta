...
(function () {
  ...
  function allWeightCandidates(text, ticket) {
    ...
    [ticket?.grossWeight,ticket?.tareWeight,ticket?.netWeight].map(Number)
      .filter(n => Number.isFinite(n) && n >= 15000 && n <= 100000).forEach(n => values.push(n));
  ...
  function weights(text, ticket) {
    ...
    for (const gross of vals) for (const tare of vals) for (const net of vals) {
      ...
      if (gross < 40000 || gross > 95000 || tare < 20000 || tare > 35000 || net < 15000 || net > 75000) continue;
  ...
  function apply(ticket, text) {
    ...
    ticket.elevatorName='Scoular - Waverly'; ticket.deliveryStreet='15379 Jasmine Road';

...
- `tickets/ticket-allocation.js` is the unified automatic allocation planner for a newly scanned/saved ticket. It plans hauling rollover, contract allocation, and review state from one state snapshot.
...
## Ticket review invariant
Review checks are advisory office workflow flags. They do not reject an otherwise valid ticket or expose warnings to the driver. Review reasons are calculated centrally so Grain Tickets and Grain Operations display the same result.

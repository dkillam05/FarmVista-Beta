...
export function installHaulingTicketWorkspace(root){
  ...
  details.addEventListener('input',e=>{if(e.target.matches('[data-hauling-filter]'))applyFilters()});details.addEventListener('change',e=>{if(e.target.matches('[data-hauling-filter]'))applyFilters();if(e.target.matches('[data-hauling-select-all]'))details.querySelectorAll('[data-hauling-select]').forEach(x=>x.checked=e.target.checked)});
  ...
  details.addEventListener('drop',async e=>{
    ...
    try{
      if(target.hasAttribute('data-hauling-unassigned')){for(const ticket of moving)if(clean(ticket.haulingJobId))await unassignTicketFromJob(ticket);return}

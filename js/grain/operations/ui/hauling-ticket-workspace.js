...
import {assignWholeTicketToJob,moveTicketPortion,moveWholeTicketToHaulingJob,moveSpotPortionToJob,unassignTicketFromJob} from '../data/grain-writes.js';
...
export function installHaulingTicketWorkspace(root){
  ...
  details.addEventListener('dragstart',e=>{const card=e.target.closest('[data-hauling-ticket-id]');if(!card)return;dragged=card.dataset.haulingTicketId;draggedSpot=!!card.dataset.haulingSpotTicketId;const selected=[...details.querySelectorAll('[data-hauling-select]:checked')].map(x=>x.dataset.haulingSelect);draggedIds=draggedSpot?[dragged]:(selected.length?selected:[dragged]);card.classList.add('dragging');e.dataTransfer.effectAllowed='move'});
  details.addEventListener('dragend',e=>{e.target.closest('[data-hauling-ticket-id]')?.classList.remove('dragging');details.querySelectorAll('.drag-over').forEach(x=>x.classList.remove('drag-over'));dragged='';draggedIds=[];draggedSpot=false});

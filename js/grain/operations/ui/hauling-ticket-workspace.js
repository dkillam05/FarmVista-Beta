...
export function installHaulingTicketWorkspace(root){
  ...
  let touchDrag=null;
  const clearTouchDrag=()=>{if(touchDrag?.ghost)touchDrag.ghost.remove();details.querySelectorAll('.drag-over').forEach(x=>x.classList.remove('drag-over'));touchDrag=null};
  const touchTarget=(x,y)=>document.elementFromPoint(x,y)?.closest?.('[data-hauling-job-drop],[data-hauling-unassign-zone]');
  details.addEventListener('touchstart',e=>{const card=e.target.closest('[data-hauling-ticket-id]');if(!card||e.target.closest('input,label'))return;const touch=e.touches[0];if(!touch)return;const id=card.dataset.haulingTicketId,spot=!!card.dataset.haulingSpotTicketId,unassigned=!!card.dataset.haulingUnassignedTicketId,selected=[...details.querySelectorAll('[data-hauling-select]:checked')].map(x=>x.dataset.haulingSelect),ids=spot?[id]:(selected.length?selected:[id]);const ghost=card.cloneNode(true);ghost.classList.add('fv-go-touch-drag-ghost');ghost.querySelectorAll('input').forEach(x=>x.remove());document.body.appendChild(ghost);touchDrag={id,ids,spot,unassigned,ghost,startX:touch.clientX,startY:touch.clientY,active:false};ghost.style.left=touch.clientX+'px';ghost.style.top=touch.clientY+'px'},{passive:true});
  details.addEventListener('touchmove',e=>{if(!touchDrag)return;const touch=e.touches[0];if(!touch)return;const dx=Math.abs(touch.clientX-touchDrag.startX),dy=Math.abs(touch.clientY-touchDrag.startY);if(!touchDrag.active&&Math.max(dx,dy)<8)return;touchDrag.active=true;e.preventDefault();touchDrag.ghost.style.left=touch.clientX+'px';touchDrag.ghost.style.top=touch.clientY+'px';details.querySelectorAll('.drag-over').forEach(x=>x.classList.remove('drag-over'));touchTarget(touch.clientX,touch.clientY)?.classList.add('drag-over')},{passive:false});
  details.addEventListener('touchend',async e=>{if(!touchDrag)return;const state=touchDrag,changed=e.changedTouches[0];if(!state.active||!changed){clearTouchDrag();return}const target=touchTarget(changed.clientX,changed.clientY);if(!target){clearTouchDrag();return}const model=getWorkspaceModel(),moving=state.ids.map(id=>model.tickets.find(t=>clean(t.id)===clean(id))).filter(Boolean),targetJobId=clean(target.dataset.haulingJobDrop);clearTouchDrag();if(!moving.length)return;try{if(target.hasAttribute('data-hauling-unassign-zone')){for(const ticket of moving){if(state.spot)await unassignSpotPortion(ticket);else if(clean(ticket.haulingJobId)||splitRows(ticket).some(x=>x.allocationType==='job'))await unassignTicketFromJob(ticket)}}else{const job=model.haulingJobs.find(j=>clean(j.id)===targetJobId);if(!job)return;for(const ticket of moving){const fresh=getWorkspaceModel(),freshTicket=fresh.tickets.find(t=>clean(t.id)===clean(ticket.id))||ticket,freshJob=fresh.haulingJobs.find(j=>clean(j.id)===clean(job.id))||job,plan=planManualHaulingMove(freshTicket,freshJob,{state:fresh});if(!plan.ok)throw new Error(`${ticket.ticketNumber||ticket.ticketNo||ticket.id}: ${plan.reason||'cannot be moved'}`);if(state.spot)await moveSpotPortionToJob(freshTicket,freshJob,spotBushels(freshTicket));else if(state.unassigned&&unassignedBushels(freshTicket)>.005)await moveUnassignedPortionToJob(freshTicket,freshJob,unassignedBushels(freshTicket));else await moveWholeTicketToHaulingJob(freshTicket,freshJob,fresh)}}details.querySelectorAll('[data-hauling-select]:checked').forEach(x=>x.checked=false);updateSelectionHelper();await rerenderPreservingView()}catch(error){alert(error?.message||'The hauling ticket assignment was not changed.')}},{passive:false});
  details.addEventListener('dragstart',e=>{const card=e.target.closest('[data-hauling-ticket-id]');if(!card)return;dragged=card.dataset.haulingTicketId;draggedSpot=!!card.dataset.haulingSpotTicketId;draggedUnassigned=!!card.dataset.haulingUnassignedTicketId;const selected=[...details.querySelectorAll('[data-hauling-select]:checked')].map(x=>x.dataset.haulingSelect);draggedIds=draggedSpot?[dragged]:(selected.length?selected:[dragged]);card.classList.add('dragging');e.dataTransfer.effectAllowed='move'});
  details.addEventListener('dragend',e=>{e.target.closest('[data-hauling-ticket-id]')?.classList.remove('dragging');details.querySelectorAll('.drag-over').forEach(x=>x.classList.remove('drag-over'));dragged='';draggedIds=[];draggedSpot=false;draggedUnassigned=false});
  details.addEventListener('dragover',e=>{const target=e.target.closest('[data-hauling-job-drop],[data-hauling-unassign-zone]');if(!target||!dragged)return;e.preventDefault();e.stopPropagation();e.dataTransfer.dropEffect='move';target.classList.add('drag-over')});
  details.addEventListener('dragleave',e=>{const target=e.target.closest('[data-hauling-job-drop],[data-hauling-unassign-zone]');if(!target)return;if(e.relatedTarget&&target.contains(e.relatedTarget))return;target.classList.remove('drag-over')});
  details.addEventListener('drop',async e=>{
    ...
    try{
      if(target.hasAttribute('data-hauling-unassign-zone')){for(const ticket of moving){if(draggedSpot)await unassignSpotPortion(ticket);else if(clean(ticket.haulingJobId)||splitRows(ticket).some(x=>x.allocationType==='job'))await unassignTicketFromJob(ticket)}details.querySelectorAll('[data-hauling-select]:checked').forEach(x=>x.checked=false);updateSelectionHelper();await rerenderPreservingView();return}
      const job=model.haulingJobs.find(j=>clean(j.id)===clean(target.dataset.haulingJobDrop));if(!job)return;
      for(const ticket of moving){const fresh=getWorkspaceModel(),freshTicket=fresh.tickets.find(t=>clean(t.id)===clean(ticket.id))||ticket,freshJob=fresh.haulingJobs.find(j=>clean(j.id)===clean(job.id))||job,plan=planManualHaulingMove(freshTicket,freshJob,{state:fresh});if(!plan.ok)throw new Error(`${ticket.ticketNumber||ticket.ticketNo||ticket.id}: ${plan.reason||'cannot be moved'}`);if(draggedSpot)await moveSpotPortionToJob(freshTicket,freshJob,spotBushels(freshTicket));else if(draggedUnassigned&&unassignedBushels(freshTicket)>.005)await moveUnassignedPortionToJob(freshTicket,freshJob,unassignedBushels(freshTicket));else await moveWholeTicketToHaulingJob(freshTicket,freshJob,fresh)}await rerenderPreservingView()

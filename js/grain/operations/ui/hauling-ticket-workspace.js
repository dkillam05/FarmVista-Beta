...
export function installHaulingTicketWorkspace(root){
  ...
  details.addEventListener('dragstart',e=>{const card=e.target.closest('[data-hauling-ticket-id]');if(!card)return;dragged=card.dataset.haulingTicketId;draggedSpot=!!card.dataset.haulingSpotTicketId;const selected=[...details.querySelectorAll('[data-hauling-select]:checked')].map(x=>x.dataset.haulingSelect);draggedIds=selected.length?selected:[dragged];card.classList.add('dragging');e.dataTransfer.effectAllowed='move'});
